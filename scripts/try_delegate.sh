...
echo
echo "  2. TALK TO IT ON THE BUILDER'S OWN PAGE  (the demo surface)"
echo "       open http://127.0.0.1:$RB_PORT/?business=Demo%20Plumbing%20Co"
echo "       The Voice panel is nano-claw: it appears only because"
echo "       RB_NANOCLAW_URL is set, and its dropdown is this node's voice"
echo "       list, read cross-origin because :$RB_PORT is declared in"
echo "       NANO_CLAW_EMBED_ORIGINS. Hold the mic and speak — the page"
echo "       captures nothing and decodes nothing; every audio decision"
echo "       happens here, and words come back for the transcript."
echo
echo "  2b. OR FROM NANO-CLAW'S OWN CONSOLE (the phone's-eye view)"
