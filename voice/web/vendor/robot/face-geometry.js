...
export function mouthGeometry(ch, bold = 1) {
  ...
  if (round > 0.02) {
    ...
    // Build the ellipse from the lens's ACTUAL lower edge, not from its control
    // point. `lowerMidY` is a quadratic control point, and a quadratic reaches
    // only halfway to it — which is the whole reason BEZIER_REACH exists. Using
    // it as an extent double-counts that reach, so the ellipse came out about
    // twice as deep as the lens it blends from and dragged the mouth off the
    // bottom of the screen: `surprised` reached y 1.066 against a screen edge at
    // 0.925, so 69 px of the mouth was cut off by the canvas boundary and the
    // remainder read as a dark smear across the binary border.
    //
    // The correct extreme is the curve's midpoint value, which the tongue block
    // below already computes for exactly this reason.
    const lowerEdge = 0.5 * cornerY + 0.5 * lowerMidY;
    const midY = (cornerY + lowerEdge) / 2;
    const ry = (lowerEdge - cornerY) / 2;
