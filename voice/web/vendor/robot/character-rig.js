...
/**
 * A cue's strength over its life, from 0 at the start to 0 at the end.
 *
 * Rise fast, then decay smoothly back to the emotion baseline — a gesture is an
 * accent, so it has to arrive quicker than it leaves. Extracted from cueOffsets
 * because mutation testing showed the shape was unguarded: replacing the whole
 * envelope with a constant 1, which turns every cue into a square pulse that
 * snaps on and off, passed all 321 tests. Only that a cue fires and eventually
 * expires was covered, never how it gets there.
 *
 * @param k progress through the cue, 0..1
 */
export function cueEnvelope(k) {
  if (!(k > 0)) return 0;
  if (k >= 1) return 0;
  return k < 0.2 ? k / 0.2 : 1 - (k - 0.2) / 0.8;
}
...
export function createRig({ config, bodyEl, canvasEl, stageEl, calibrationEl = null }) {
  ...
  function frame(now) {
    ...
    if (sampleAudio() && audioLevel > 0.012) {
      // Snapped, not sprung. `audioLevel` is already the output of the
      // asymmetric envelope follower below — 0.55 attack, 0.18 release, tuned so
      // the mouth opens on the transient and closes unhurriedly. That IS the
      // smoothing; a spring on top of it smooths an already-smoothed signal and
      // only adds lag. There is nothing to interpolate toward either, since a
      // fresh measurement arrives every frame.
      springs.snap({
  ...
  return {
    ...
    /**
     * The host-push path, for a caller that measures its own audio.
     *
     * It used to write a spring TARGET from the raw level and nothing else,
     * which failed twice over: no envelope follower, so the value was as noisy
     * as whatever the host supplied, and a 350ms spring in front of it, so the
     * mouth could not follow speech. Measured against a known envelope, the
     * targets swung 0.10..0.85 while the rendered mouth moved only 0.43..0.52 —
     * about a fifth of the commanded movement at real speech rates.
     *
     * It now shares the analyser path's design exactly: the same follower, the
     * same mapping, snapped. One behaviour whichever way audio arrives.
     */
    pushAudioFrame({ level = 0, speaking = true } = {}) {
      const l = Math.min(1, Math.max(0, Number(level) || 0));
      const target = speaking ? l : 0;
      audioLevel += (target - audioLevel) * (target > audioLevel ? 0.55 : 0.18);
      springs.snap({ mouthOpen: speaking ? 0.08 + audioLevel * 0.78 : 0.05 });
    },
    setAudioLevel(level) {
