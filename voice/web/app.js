...
import { DocumentsUI } from './documents.js?v=0.4.32';
  ...
  try {
    ...
    if (!_diagEl) {
      ...
      _diagEl.addEventListener('click', function () {
        try {
          navigator.clipboard.writeText(_diagEl.textContent);
        } catch (_e) {}
      });
...
const CONSOLE_ACCENT =
  getComputedStyle(document.documentElement).getPropertyValue('--accent').trim() || '#ff775f';
const CONSOLE_ACCENT_SECONDARY =
  getComputedStyle(document.documentElement).getPropertyValue('--accent-2').trim() || '#ffca5c';
...
function validStoredVisualizationProfile(profile) {
  ...
  if (!profile.settings || typeof profile.settings !== 'object' || Array.isArray(profile.settings))
    return false;
  if (!profile.camera || typeof profile.camera !== 'object' || Array.isArray(profile.camera))
    return false;
...
function loadVisualizationSettings() {
  ...
  try {
    ...
    var scene =
      stored && Object.prototype.hasOwnProperty.call(VISUALIZATION_SCENES, stored.scene)
        ? stored.scene
        : 'custom';
    return {
      scene: scene,
...
async function switchRenderer(id) {
  ...
  if (next === 'mascot') {
    ...
    const { createMascotRenderer } = await import('./mascot-renderer.js?v=0.4.32');
    ...
    const { createRobotRenderer } = await import('./robot-renderer.js?v=0.4.32');
...
window.VoiceEmotion = {
  set: function (name, opts) {
    emotionAuto = false;
    return setVisualEmotion(name, opts);
  },
  presence: setVisualPresence,
  auto: function (on) {
    emotionAuto = on !== false;
    return emotionAuto;
  },
  state: function () {
    return Object.assign({ auto: emotionAuto }, emotionState);
  },
...
function effectiveVisualizationSettings() {
  ...
  return Object.assign({}, base, {
    ...
    energy: visualizationSpeaking ? Math.max(0.58, base.energy) : base.energy,
    primary: primary,
    secondary: secondary,
  });
}
...
function visualizationRangeLabel(input) {
  ...
  if (input.dataset.cubeSetting === 'speed') {
    return (
      Math.round(((value - Number(input.min)) / (Number(input.max) - Number(input.min))) * 100) +
      '%'
    );
  }
...
function matchingVisualizationPalette() {
  return (
    Object.keys(VISUALIZATION_PALETTES).find(function (name) {
      var colors = VISUALIZATION_PALETTES[name];
      return (
        colors[0].toLowerCase() === visualizationSettings.primary.toLowerCase() &&
...
function syncVisualizationControls() {
  ...
  cubePolygonSides
    .closest('.pipe-row')
    .classList.toggle('is-disabled', visualizationSettings.formation !== 'polygon');
...
function storeVisualizationChange(partial) {
  visualizationSettings = normalizeVisualizationSettings(
    Object.assign({}, visualizationSettings, partial)
  );
...
function applyPhoneConfig(cfg) {
  ...
  if (cfg.active_calls > 0) {
    phoneCallStatus.textContent =
      '● live · ' + cfg.active_calls + (cfg.active_calls === 1 ? ' caller' : ' callers');
...
function phoneControlsUnavailable() {
  [phoneVoiceSelect, phoneModelSelect, phoneSttSelect, phoneSpeedSlider].forEach(function (el) {
    el.disabled = true;
  });
  var o = document.createElement('option');
  o.textContent = 'n/a (phone disabled)';
...
function loadPhoneConfig() {
  fetch('/api/phone/config')
    .then(function (r) {
      if (!r.ok) {
        throw new Error('phone disabled');
      }
      return r.json();
    })
    .then(applyPhoneConfig)
    .catch(phoneControlsUnavailable);
}
...
function findEnabledSelectOption(selectEl, value) {
  ...
  return (
    Array.from(selectEl.options || []).find(function (option) {
...
function renderFlowConfig(config) {
  var options = Array.isArray(config.options)
    ? config.options.filter(function (option) {
        return (
          option && typeof option.id === 'string' && option.id && typeof option.label === 'string'
        );
      })
    : [];
...
function renderRegionModelConfig(config) {
  ...
  pageLog(
    'region model selection active=' +
      ...
      activeRegionModel +
      ' source=' +
      (appliedResolution.fallback ? appliedResolution.source : stagedResolution.source) +
      ' fallback=' +
      (stagedResolution.fallback || appliedResolution.fallback)
  );
}

function loadRegionModelConfig() {
  if (!currentEnabledSelectOption(regionModelSelect)) regionModelSelect.disabled = true;
  return fetch('/api/voice/region-model')
    .then(function (r) {
...
function setAudioUnavailable(reason) {
  setAudioState('unavailable', reason + (linkReady ? ' · type below' : ''));
}
...
function createBargeInController() {
  if (
    typeof window.BargeInDetector !== 'function' ||
...
function sampleBargeIn(rms, now) {
  ...
  if (after.adjustments !== before.adjustments) {
    adjustmentMessage =
      after.currentThreshold > before.currentThreshold
...
var deepStatusLine = null;

function updateDeepStatus(message) {
  if (!deepStatusLine || !deepStatusLine.isConnected) {
    deepStatusLine = appendSystemLine(message, 'deep');
  } else {
...
function updateFlowBenchmarks(state) {
  var supervisorMs =
    typeof state.supervisor_ms === 'number' && Number.isFinite(state.supervisor_ms)
      ? state.supervisor_ms
      : null;
  ...
  if (typeof state.supervisor_ms === 'number' && Number.isFinite(state.supervisor_ms)) {
    ...
    appendSystemLine(
      'Scheduler turn ' +
        ...
        maxLabel +
        ' · supervisor ' +
        formatLatency(state.supervisor_ms)
    );
  }
...
function renderFlowState(state) {
  ...
  var duration =
    typeof slots.duration_minutes === 'number'
      ? slots.duration_minutes + ' minutes'
      : slots.duration_minutes;
  setFlowSlot(flowSlotDuration, flowSlotDurationValue, slots.duration_minutes, duration);

  var turnsUsed = Number.isInteger(state.turns_used) ? state.turns_used : 0;
  var maxTurns = Number.isInteger(state.max_turns) ? state.max_turns : 0;
  flowBudget.textContent = 'turn ' + turnsUsed + ' / ' + maxTurns;
  flowModel.textContent = activeRegionModel ? 'model ' + activeRegionModel : 'model —';
  flowLatency.textContent =
    typeof state.supervisor_ms === 'number'
...
function addDebugEntry(info) {
  ...
  fields.forEach(function (pair, i) {
    ...
    value.className =
      pair[0] === 'tok' ? 'debug-tokens' : pair[0] === 'dur' ? 'debug-duration' : '';
...
function showDebugDetail(info) {
  ...
  var details = [
    ...
    },
    {
      ...
      value:
        info.tokenUsage && (info.tokenUsage.cacheRead || info.tokenUsage.cacheWrite)
...
function renderVoiceOptions(uiCatalog) {
  ...
  resolveEnabledSelectSelection(phoneVoiceSelect, stagedPhoneResolution.value, defaultVoiceId);
...
function loadModels() {
  ...
  return fetch('/api/models')
    ...
    .then(function (data) {
      ...
      def.value = '';
      def.textContent = 'server default (' + serverDefault + ')';
      ...
      var requestedPhoneModel =
        phonePendingModel !== null ? phonePendingModel : phoneModelSelect.value;
      var stagedPhoneResolution = resolveEnabledSelectSelection(
    ...
    .catch(function (err) {
      ...
      if (!modelsLoaded && !modelsRetryTimer) {
        modelsRetryTimer = setTimeout(function () {
          modelsRetryTimer = null;
          loadModels();
        }, 1500);
      }
...
sttSelect.addEventListener('change', function () {
  currentStt = sttSelect.value;
  localStorage.setItem(LS_STT, currentStt);
  sendMsg('set_stt', { size: currentStt });
});
if (analysisStyleToggle) {
  analysisStyleToggle.addEventListener('change', function () {
    currentAnalysisStyle = analysisStyleToggle.checked ? 'principle_graph' : 'topic_map';
    localStorage.setItem(LS_ANALYSIS_STYLE, currentAnalysisStyle);
    sendMsg('set_analysis_style', { analysisStyle: currentAnalysisStyle });
...
function connect() {
  ...
  socket.onopen = function () {
    ...
    if (modelsLoaded) syncModelToServer();
    else loadModels();
    if (voicesLoaded) pushVoice();
    else loadVoices();
  };
    ...
    case 'hello_ack':
      ...
      pageLog(
        'hello_ack wsAudio=' +
          wsAudioEnabled +
          ' -> ' +
          (wsAudioEnabled ? 'WS-audio' : 'WebRTC') +
          ' path'
      );
...
function enqueueWsAudioFrame(frame) {
  ...
  try {
    wsAudioPlayer.enqueue(frame);
  } catch (error) {
    console.error('Invalid agent PCM frame', error);
  }
}
...
async function startWsAudio(generation, announcedFormat) {
  ...
  if (
    micFormat.format !== 'pcm_s16le' ||
    micFormat.sampleRate !== 16000 ||
    micFormat.channels !== 1 ||
    micFormat.frameSamples !== 320 ||
    agentFormat.format !== 'pcm_s16le' ||
    agentFormat.sampleRate !== 48000 ||
    agentFormat.channels !== 1 ||
    agentFormat.frameSamples !== 960
  ) {
  ...
  try {
    ...
    pageLog(
      'getUserMedia FAILED: ' + (_error && _error.name) + ' ' + ((_error && _error.message) || '')
    );
...
async function startWebRTC(generation) {
  ...
  function syncWebRtcReadiness() {
    ...
    if (micStream && iceReady) {
      ...
      setAudioState('connecting', micStream ? 'Connecting voice' : 'Mic permission pending');
    }
  }
...
function finishAutomaticTurn(reason) {
  ...
  setPhoneStatus(
    reason === 'maximum_duration'
      ? 'Maximum turn reached; transcribing...'
      : 'Transcribing phone audio...'
  );
